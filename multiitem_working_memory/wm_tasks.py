...
from purias_utils.multiitem_working_memory.stimulus_board import StimulusBoardBase, ColouredSquaresBoard
...
class MultiItemWMTaskBase:
    ...
    def generate_epoch_duration(self, *args, **kwargs) -> Dict[str, int]:
        """
...
class SimpleMultiItemWMChangeDetectionTask(MultiItemWMTaskBase):
    ...
    def generate_epoch_duration(self, epoch_name): 
...
class SimpleMultiItemWMDelayedEstimationTask(MultiItemWMTaskBase):
        ...
        if epoch_name in ['prestim', 'stim', 'wm']:
            ...
            for board in boards:
                ...
                for i in range(len(copied_board.stimuli)):
                    ...
                    changed_stim = changed_stim.set('location', *copied_board.stimuli[selected_stim_idx].features['location'].value)
    ...
    def generate_sensory_mask(self, boards, epoch_name):
        ...
        if epoch_name in ['stim', 'cue', 'resp']:
    ...
    def generate_target_masks(self, boards, epoch_name):
        ...
        # TODO: MAKE PARAMETERISABLE
        if epoch_name == 'resp':
            target_masks = [[1., 0.] for _ in boards]
        else:
            target_masks = [[1., self.epoch_kwargs['fixation_mag_div']] for _ in boards]
        return target_masks
