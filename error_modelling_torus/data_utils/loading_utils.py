...
def load_experimental_data(dataset_name, train_indices_seed, train_indices_path, M_batch, M_test_per_set_size, data_subselection_args, **kwargs) -> MultipleSetSizesActivitySetDataGeneratorEnvelopeBase:
    ...
    if dataset_name == 'bays2009':
        dataset_generator = Bays2009MultipleSetSizesEnvelope(M_batch, M_test_per_set_size, participant_id = data_subselection_args.participant_id, stimulus_exposure_id = data_subselection_args.stimulus_exposure_id)
    elif dataset_name == 'mcmaster2022_e1_oricue':
        dataset_generator = McMaster2022ExperimentOneEnvelope(M_batch, M_test_per_set_size, subtask = 'oricue', subjects = data_subselection_args.subjects, stim_strengths = data_subselection_args.stim_strengths)
    elif dataset_name == 'mcmaster2022_e2_dircue':
        dataset_generator = McMaster2022ExperimentTwoEnvelope(M_batch, M_test_per_set_size, subtask = 'dircue', subjects = data_subselection_args.subjects, stim_strengths = data_subselection_args.stim_strengths)
    elif dataset_name == 'mcmaster2022_e1_loccue':
        dataset_generator = McMaster2022ExperimentOneEnvelope(M_batch, M_test_per_set_size, subtask = 'loccue', subjects = data_subselection_args.subjects, stim_strengths = data_subselection_args.stim_strengths)
    elif dataset_name == 'vandenberg2012_color':
        dataset_generator = VanDenBerg2012ColourEnvelope(M_batch, M_test_per_set_size, subjects = None)
    elif dataset_name == 'vandenberg2012_orientation':
        dataset_generator = VanDenBerg2012OrientationEnvelope(M_batch, M_test_per_set_size, subjects = None)
...
def load_synthetic_data(dataset_generator, synthetic_data_path, synthetic_data_code):
    ...
    for N, dg in dataset_generator.data_generators.items():
        ...
        assert dg.all_errors.shape == synthetic_errors.shape
